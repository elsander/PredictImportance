EmpiricalViolins <- function(pathToResults){
    ...
    for(i in 1:length(webnames)){
        ...
        serem <- sqrt(diag(vcov(empir.rem)))
        sepert <- sqrt(diag(vcov(empir.pert)))
...
readline("Simulation and analysis for all empirical networks will run now. This may take from several hours to a couple days! Press any key to continue:")
